...
const fetchAvailableUsers = async () => {
  ...
  try {
    ...
    if (userRole === 'customer') {
      console.log('🔍 Filtering for customer role');
      filteredUsers = filteredUsers.filter(u => 
        u.role === 'vendor' || u.role === 'rider' || u.role === 'admin'
      );
    } else if (userRole === 'vendor') {
      console.log('🔍 Filtering for vendor role');
      filteredUsers = filteredUsers.filter(u => 
        u.role === 'customer' || u.role === 'rider' || u.role === 'admin'
      );
    } else if (userRole === 'business') {
      console.log('🔍 Filtering for business role');
      console.log('Allowed roles: rider, admin');
      console.log('Before filter count:', filteredUsers.length);
      
      filteredUsers = filteredUsers.filter(u => {
        const allowed = u.role === 'rider' || u.role === 'admin';
        console.log(`User ${u.name} (${u.role}) ${allowed ? '✓ allowed' : '✗ filtered out'}`);
        return allowed;
      });
      
      console.log('After filter count:', filteredUsers.length);
    } else if (userRole === 'rider') {
      console.log('🔍 Filtering for rider role');
      filteredUsers = filteredUsers.filter(u => 
